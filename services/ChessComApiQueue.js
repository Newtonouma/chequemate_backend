...
class ChessComApiQueue {
  constructor() {
    ...
    this.cache = new Map();
    this.cacheTTL = {
      playerStats: 15 * 60 * 1000, // 15 minutes for player stats
      gameArchives: 30 * 60 * 1000, // 30 minutes for game archives list
      monthlyGames: 5 * 60 * 1000,  // 5 minutes for monthly games
      playerProfile: 30 * 60 * 1000 // 30 minutes for player profile
    };
    
    // Create axios instance with proper identification
    this.api = axios.create({
      headers: {
        'User-Agent': 'jkuat-university-chess-club-tool/0.1 (username: rookwitdahooks; contact: rolljoe42@gmail.com)',
    ...
    console.log('🔄 [CHESS_API_QUEUE] Initialized with sequential request processing');
    console.log('🔄 [CHESS_API_QUEUE] User-Agent: college-club-members-tool/0.1 (username: rookwitdahooks; contact: rolljoe42@gmail.com)');
  ...
  cleanCache() {
    ...
    if (cleaned > 0) {
      console.log(`🗄️ [CHESS_API_QUEUE] Cleaned ${cleaned} expired cache entries. Cache size: ${this.cache.size}`);
    }
  }
  ...
  setRateLimit(durationMs = 5 * 60 * 1000) {
    ...
    console.log(`🚫 [CHESS_API_QUEUE] Rate limited! Pausing for ${minutes} minutes until ${timeString}`);
    
    // Clear queue to prevent further rate limiting
    const queueLength = this.queue.length;
    this.queue = [];
    console.log(`🚫 [CHESS_API_QUEUE] Cleared ${queueLength} pending requests due to rate limiting`);
  }
  ...
  async request(config, cacheType = 'default') {
    ...
    if (rateLimitRemaining) {
      console.log(`🚫 [CHESS_API_QUEUE] Rate limited for ${rateLimitRemaining} more minutes, rejecting request`);
      throw new Error(`Chess.com API rate limited for ${rateLimitRemaining} more minutes`);
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      console.log(`📝 [CHESS_API_QUEUE] Request queued: ${config.url}. Queue length: ${this.queue.length}`);
  ...
  async processQueue() {
    ...
    if (timeSinceLastRequest < this.requestDelay) {
      ...
      console.log(`⏱️ [CHESS_API_QUEUE] Waiting ${waitTime}ms before next request (queue: ${this.queue.length})`);
    ...
    try {
      ...
      if (item.config.method === 'get') {
        ...
        console.log(`🗄️ [CHESS_API_QUEUE] Cached response for ${item.config.url} (TTL: ${ttl/1000}s)`);
      }
      ...
      if (this.consecutiveSuccesses > 5 && this.requestDelay > 1500) {
        ...
        console.log(`⚙️ [CHESS_API_QUEUE] Adjusted delay to ${this.requestDelay}ms after ${this.consecutiveSuccesses} successes`);
      }
      ...
      const isRateLimitError = (
        (error.response && (error.response.status === 410 || error.response.status === 429)) ||
      ...
      if (isRateLimitError) {
        console.log(`🚫 [CHESS_API_QUEUE] Rate limit detected on ${item.config.url}`);
        ...
        console.log(`⚠️ [CHESS_API_QUEUE] Non-rate-limit error, adjusted delay to ${this.requestDelay}ms`);
      }
  ...
  clearCache(pattern = null) {
    if (pattern) {
      ...
      console.log(`🗄️ [CHESS_API_QUEUE] Cleared ${cleared} cache entries matching '${pattern}'`);
    } else {
  ...
  cleanup() {
    console.log(`🧹 [CHESS_API_QUEUE] Cleaning up. Queue length: ${this.queue.length}, Cache size: ${this.cache.size}`);
