  ...
  socket.on("challenge-accept", async (data) => {
    try {
      ...
      if (challenge.bet_amount && challenge.bet_amount > 0) {
        console.log(
          `💳 [SOCKET] Processing payment challenge with amount: ${challenge.bet_amount}`
        );

        // Get phone numbers - prioritize challenger_phone from database
        const challengerPhone =
          challenge.challenger_phone || data.paymentDetails?.phoneNumber;
        const opponentPhone =
          challenge.opponent_phone || data.opponentPhoneNumber;
        ...
        console.log(
          `💡 [SOCKET] Payment initiation will occur when players redirect to chess platform`
        );
      }
      ...
      if (challengeQuery.rows.length > 0) {
        ...
        if (challengeData.bet_amount > 0) {
          if (isChallenger && challengeData.challenger_phone) {
            console.log(
              `💰 Initiating deposit for challenger: ${challengeData.challenger_phone}, amount: ${challengeData.bet_amount}`
            );
            try {
              ...
              console.log(
                `💰 Challenger payment initiation result:`,
                paymentResult.success ? "SUCCESS" : "FAILED",
                paymentResult.error || ""
              );
            } catch (paymentError) {
              console.error(
                `❌ Challenger payment initiation error:`,
                paymentError
              );
            }
          }
          ...
          if (!isChallenger && challengeData.opponent_phone) {
            console.log(
              `💰 Initiating deposit for opponent: ${challengeData.opponent_phone}, amount: ${challengeData.bet_amount}`
            );
            try {
              const paymentResult = await paymentService.initiateDeposit(
              ...
              console.log(
                `💰 Opponent payment initiation result:`,
                paymentResult.success ? "SUCCESS" : "FAILED",
                paymentResult.error || ""
              );
            } catch (paymentError) {
              console.error(
                `❌ Opponent payment initiation error:`,
                paymentError
              );
            }
          }
