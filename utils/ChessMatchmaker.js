...
class ChessMatchmaker {
  ...
  scoreOpponent(userFeatures, opponentFeatures) {
    ...
    let timeScore = 0;
    for (const [timeControl, userPref] of Object.entries(
  ...
  async findBestOpponent(
    challengerUsername,
    availableUsers,
    challengerPlatform = "chess.com"
  ) {
    try {
      // Build features for challenger
      const challengerFeatures = await this.buildUserFeatures(
        challengerUsername,
        challengerPlatform
      );
      ...
      for (const user of availableUsers) {
        ...
        try {
          ...
          const opponentFeatures = await this.buildUserFeatures(
            platformUsername,
            challengerPlatform
          );
          ...
          const ratingDiff = Math.abs(
            challengerFeatures.rating - opponentFeatures.rating
          );
          if (ratingDiff > 300) continue;

          // Check for common time controls
          const hasCommonTimeControl = Object.keys(
            challengerFeatures.timePreferences
          ).some((tc) => opponentFeatures.timePreferences[tc] > 0);
          if (!hasCommonTimeControl) continue;

          const score = this.scoreOpponent(
            challengerFeatures,
            opponentFeatures
          );
  ...
  explainMatch(challengerFeatures, opponentFeatures) {
    ...
    if (
      Math.abs(challengerFeatures.streak) < 2 &&
      Math.abs(opponentFeatures.streak) < 2
    ) {
      reasons.push("Both players in balanced form");
    }

    // Find common time controls
    const commonTimeControls = Object.keys(
      challengerFeatures.timePreferences
